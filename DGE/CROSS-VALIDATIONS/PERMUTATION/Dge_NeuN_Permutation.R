...
permutation_NeuN <- vector("list",length = B)
for (i in 1:B)
    {
        permutation_NeuN[[i]] <- data.frame(matrix(nrow=ncol(Y.b[[i]]), ncol=1, dimnames = list(colnames(Y.b[[i]]), c("Anova"))))
        {
        for (j in 1:ncol(Y.b[[i]])) 
            ...
            Model[[i]]=tryCatch(anova(lm(as.formula(paste("Y.b[[i]][,j] ~ ", paste(colnames(pd),collapse = " + "))), data = pd)),warning =  function(w) w)
                ...
                if(typeof(Model[[i]]) == "list"){
                    permutation_NeuN[[i]][j,"Anova"] = Model[[i]]$"Pr(>F)"[1]
                }
            }
        }
    }
save(permutation_NeuN,file="permutation_NeuN.RData")

myfiles <- vector("list", length = B)
for (i in 1:B)
    {
      permutation_NeuN[[i]] <- permutation_NeuN[[i]] %>% rownames_to_column('Gene')
      myfiles[[i]] <- permutation_NeuN[[i]][permutation_NeuN[[i]]$Anova < 0.05,]
    }

df <- data.frame(Perm = sapply(myfiles, nrow))
obs <- read.table("OUTPUTS_EXONS_NEUN/SPECIES_DGE.txt")

pdf("Permutation_Histogram_NeuN.pdf",width=4,height=2,useDingbats=FALSE)
...
AnovaP <- do.call(cbind,permutation_NeuN) %>% as.data.frame()
AnovaP$P_mean <- rowMeans(AnovaP)


df <- data.frame(Obs = -log10(obs$Anova_Species), Boot = -log10(AnovaP$P_mean))

pdf("Permutation_Anova_NeuN.pdf",width=4,height=4,useDingbats=FALSE)

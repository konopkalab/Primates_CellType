...
####################################
# Load Inputs and filter for OLIG2+ #
####################################
dir.create("OUTPUTS_EXONS_OLIG2/")
load(list.files(pattern = "OLIG2",path = "PRIMATE_EXPRESSION_INPUT",full.names = TRUE))

data <- HCR_OLIG2_Exons
pd <- HCR_OLIG2_pd
...
write.table(p, "OUTPUTS_EXONS_OLIG2/OLIG2_Primates_Exons_CPM.txt",sep="\t",quote=F)

#########################
# Variance explained    #
#########################
var <- VarExp(p,pd,5,FALSE)
pdf("OUTPUTS_EXONS_OLIG2/Variance_Explained_OLIG2.pdf",width=8,height=6)
...
#         PCA           #
#########################
pdf("OUTPUTS_EXONS_OLIG2/OLIG2_Primates_PCA.pdf",width=5,height=5,useDingbats=FALSE)
...
write.table(adj.residuals, "OUTPUTS_EXONS_OLIG2/OLIG2_Primates_AdjExp.txt",sep="\t",quote=F)
pAdj <- adj.residuals
...
write.table(SPECIES_DGE, "OUTPUTS_EXONS_OLIG2/SPECIES_DGE.txt",sep="\t",quote=F)
...
write.table(HsapVsPanTro_DGE, "OUTPUTS_EXONS_OLIG2/HsapVsPanTro_DGE.txt",sep="\t",quote=F)
...
write.table(HsapVsRheMac_DGE, "OUTPUTS_EXONS_OLIG2/HsapVsRheMac_DGE.txt",sep="\t",quote=F)
...
write.table(PanTroVsRheMac_DGE, "OUTPUTS_EXONS_OLIG2/PanTroVsRheMac_DGE.txt",sep="\t",quote=F)

################
# DGE database #
################
files = list.files(path = "OUTPUTS_EXONS_OLIG2/",pattern = 'DGE.txt',full.names = TRUE)
...
write.table(res, "OUTPUTS_EXONS_OLIG2/OLIG2_PRIMATES_DGE.txt",sep="\t",quote=F)
write.xlsx(res, file="OUTPUTS_EXONS_OLIG2/OLIG2_PRIMATES_DGE.xlsx",sheetName = "GeneSetsDb_OLIG2",row.names=FALSE)

#########################
# Species-specific DGE  #
#########################
files = list.files(path = "OUTPUTS_EXONS_OLIG2/",pattern = '_PRIMATES_DGE.txt',full.names = TRUE)
...
files = list.files(path = "OUTPUTS_EXONS_OLIG2/",pattern = '_CPM.txt',full.names = TRUE)
exp = lapply(files, read.table,header=T,sep="\t")[[1]]

mat <- exp[rownames(exp)%in%dfH$Gene,]
pdf("OUTPUTS_EXONS_OLIG2/Human_Genes_Heatmap.pdf",width=4,height=4)
...
write.table(dfH, "OUTPUTS_EXONS_OLIG2/OLIG2_PRIMATES_DGE_HUMAN_SPECIFIC.txt",sep="\t",quote=F)
write.xlsx(dfH, file="OUTPUTS_EXONS_OLIG2/OLIG2_PRIMATES_DGE_SIGN.xlsx",sheetName = "HUMAN SPECIFIC",row.names=FALSE,append=TRUE, showNA=FALSE)
...
write.table(dfACC, "OUTPUTS_EXONS_OLIG2/OLIG2_PRIMATES_DGE_PRIMATE_ACCELERATED.txt",sep="\t",quote=F)
write.xlsx(dfACC, file="OUTPUTS_EXONS_OLIG2/OLIG2_PRIMATES_DGE_SIGN.xlsx",sheetName = "PRIMATE ACCELERATED",row.names=FALSE,append=TRUE, showNA=FALSE)
...
pdf("OUTPUTS_EXONS_OLIG2/Chimp_Genes_Heatmap.pdf",width=4,height=4)
pheatmap(mat,cluster_cols=T,scale="row",color = colorRampPalette(c("navy", "white", "firebrick3"))(50),annotation=pd, show_rownames = F,show_colnames=F)
dev.off()


write.table(dfC, "OUTPUTS_EXONS_OLIG2/OLIG2_PRIMATES_DGE_CHIMP_SPECIFIC.txt",sep="\t",quote=F)
write.xlsx(dfC, file="OUTPUTS_EXONS_OLIG2/OLIG2_PRIMATES_DGE_SIGN.xlsx",sheetName = "CHIMP SPECIFIC",row.names=FALSE,append=TRUE, showNA=FALSE)
...
pdf("OUTPUTS_EXONS_OLIG2/Rhesus_Genes_Heatmap.pdf",width=4,height=4)
pheatmap(mat,cluster_cols=T,scale="row",color = colorRampPalette(c("navy", "white", "firebrick3"))(50),annotation=pd, show_rownames = F,show_colnames=F)
dev.off()

write.table(dfR, "OUTPUTS_EXONS_OLIG2/OLIG2_PRIMATES_DGE_MACAQUE_SPECIFIC.txt",sep="\t",quote=F)
write.xlsx(dfR, file="OUTPUTS_EXONS_OLIG2/OLIG2_PRIMATES_DGE_SIGN.xlsx",sheetName = "MACAQUE SPECIFIC",row.names=FALSE,append=TRUE, showNA=FALSE)

####################
# WGCNA input data #
####################
files = list.files(path = "OUTPUTS_EXONS_OLIG2/",pattern = '_AdjExp.txt',full.names = TRUE)
...
pheno <- tmp
save(expAdj,expQuant,pheno,pd,file = "OUTPUTS_EXONS_OLIG2/WGCNA_EXONS_OLIG2.RData")

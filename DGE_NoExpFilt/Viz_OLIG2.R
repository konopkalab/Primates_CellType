...
pdf("OLIG2_Viz/SCATTER_Estimates_Hsap_OLIG2.pdf",width=6,height=6,useDingbats=FALSE)
...
pdf("OLIG2_Viz/SCATTER_Estimates_Hsap_OLIG2_NoLabels.pdf",width=4,height=4,useDingbats=FALSE)
ggscatter(hsp, x = "Estimate_HsapVsPanTro", y = "Estimate_HsapVsRheMac",
   color = "Direction",palette=c("steelblue","red"),size = 0.5,
   cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
   cor.coeff.args = list(method = "spearman",label.sep = "\n"))+
...
pdf("OLIG2_Viz/OLIG2_Primates_PCA.pdf",width=6,height=6,useDingbats=FALSE)
...
ggtitle("Principal Component Analysis: OLIG2")+
...
pdf("OLIG2_Viz/OLIG2_PRIMATES_DGE_MEANEXP.pdf",width=4,height=3,useDingbats=FALSE)
ggline(tmp.m, x = "Species", y = "value", color = "Direction",
add = c("mean_se"), palette = c("steelblue", "red"))+
theme(axis.text.x = element_text(angle = 45, hjust = 1))+
theme(legend.position="right")+
border(color = "magenta4")+
ggtitle("DGE Species: OLIG2")+
xlab("")+ 
...
# MAplot
dge <- AllStat
dge$Mean <- rowMeans(expAdj[,grep("Hsap",colnames(expAdj))])
dge$Sign <- ifelse(dge$Gene %in% Hspecific$Gene,"Sign","NotDge")

pdf("OLIG2_Viz/MAplot.pdf",width=4,height=4,useDingbats=FALSE)
ggscatter(dge, 
   x = "Mean", 
   y = "Estimate_HsapVsPanTro",
   color = "Sign", 
   palette=c("grey","blue"),
   alpha=0.5# Points color, shape and size
   )+
theme(legend.position="none")+
...
pdf("OLIG2_Viz/RADAR_OLIG2.pdf",width=5,height=5,useDingbats=FALSE)
...
ggtitle("Species Specific DGE: OLIG2")+
border(color = "magenta4")+
...
pdf("OLIG2_Viz/RADAR_OLIG2_NoDGE.pdf",width=5,height=5,useDingbats=FALSE)
...
cool <- c("CNTNAP4","DOCK1","SLITRK5","TCF20")
...
pdf("OLIG2_Viz/Boxplot_Coolguys.pdf",width=7,height=3,useDingbats=FALSE)

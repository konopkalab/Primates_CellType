...
server <- function(input, output) {
  ...
  observe({
    output$cell_class<-renderUI({
      ...
      selectInput("cell_class", "Select the class", choices = colnames(gTable_data))
  ...
  output$vioPlot<-renderPlot({
    ...
    vPlot=ggviolin(plot_data, x = "Category", y = "Expression", fill = "Category",
      palette = c("#56B4E9", "#999999","#7FFF00"),
      add = "boxplot", add.params = list(fill = "white"))+
      theme_classic() +
      theme(legend.position="none",plot.title = element_text(lineheight=.8, face="bold"))+ 
      scale_x_discrete(labels=c("Type2" = "CTL","Type3" = "panTro","Type4"="rheMac"))+
      stat_compare_means(comparisons = tocompare, label = "p.signif",method="t.test")+
      stat_compare_means(method = "anova",label.y = max(plot_data$Expression)*1.1,size = 6)+
      xlab("")+
      ylab("log2(CPM+1)")+
  ...
  observe({
    output$boxPlot<-renderPlot({
      ...
      color = "Category", palette =c("#56B4E9", "#999999","#7FFF00"),
...
ui <- fluidPage(
  ...
  # Application title
  titlePanel(h1("Comparative Genomic")),
  ...
  sidebarLayout(
    sidebarPanel(
        ...
        numericInput("thres", 'Choose expression threshold', 0, min = 0, max = NA), width = 3),
